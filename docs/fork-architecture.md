...
This document describes the target capsule map for the fork rebuild.
During early reapply phases on a fresh upstream branch, some referenced seam files or owned subtrees may not exist yet; treat those paths as the intended landing zones for later phases rather than proof that the current branch is already fully rebound.
