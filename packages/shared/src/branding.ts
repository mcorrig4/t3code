...
function replaceMetaTagContent(tag: string, content: string): string {
  if (/\scontent="[^"]*"/.test(tag)) {
    return tag.replace(/\scontent="[^"]*"/, ` content="${content}"`);
  }
...
function upsertMetaTag(html: string, name: string, content: string): string {
  ...
  if (metaRegex.test(html)) {
    return html.replace(metaRegex, (match) => replaceMetaTagContent(match, content));
  }

  return html.replace("</head>", `  <meta name="${name}" content="${content}" />\n  </head>`);
}

function ensureManifestLink(html: string): string {
...
function ensureBrandingStyleTag(html: string, branding: AppBranding): string {
  const styleTag = renderBrandingStyleTag(branding);
  if (html.includes(BRANDING_HTML_MARKER)) {
    return html.replace(BRANDING_HTML_MARKER, styleTag);
  }

  if (/id="t3-branding-vars"/.test(html)) {
    return html.replace(/<style id="t3-branding-vars">[\s\S]*?<\/style>/, styleTag);
  }

  return html.replace("</head>", `  ${styleTag}\n  </head>`);
}

function maybeUpdateDocumentTitle(html: string, branding: AppBranding): string {
  if (!branding.hostVariant) {
    return html;
  }

  if (/<title>[\s\S]*?<\/title>/i.test(html)) {
    return html.replace(/<title>[\s\S]*?<\/title>/i, `<title>${branding.applicationName}</title>`);
  }

  return html.replace("</head>", `  <title>${branding.applicationName}</title>\n  </head>`);
}
...
export function applyBrandingToHtml(html: string, branding: AppBranding): string {
  ...
  output = output.replace(/<meta([^>]*name="theme-color"[^>]*)>/gi, (match) =>
    replaceMetaTagContent(match, branding.themeColor),
  );

  output = upsertMetaTag(output, "application-name", branding.applicationName);
  output = upsertMetaTag(output, "apple-mobile-web-app-title", branding.appleMobileWebAppTitle);
  output = ensureManifestLink(output);
  output = maybeUpdateDocumentTitle(output, branding);
  return ensureBrandingStyleTag(output, branding);
}
