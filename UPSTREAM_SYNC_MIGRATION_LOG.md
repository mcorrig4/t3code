...
- Sync branch: `sync/upstream-20260426`
- Upstream base commit: `ada410bc0edc4d4fda462a9c6fcffbf224b2a3ce`
- Local fork reference branch: `main`

## Latest Reapply Notes

- Phase 1 restores the governance stack, enhancement ledger, branch guardrails, and fork-engineering skill first so later reapply work has the correct source of truth before feature code lands.
- This rebuild explicitly drops stale pending approval/user-input cleanup from the retained enhancement inventory unless a later comparison proves a remaining upstream gap.
...
The phase wrappers exist for continuity; the capsule seams are the long-term target source of truth for this rebuild, even if some seam files land in later phases on a fresh upstream branch.
...
- Notes:
  ...
  - the target port keeps fork-only settings in `t3code:fork-settings:v1`, with upstream dirty/reset composition flowing through `apps/web/src/settings/resetPlan.ts` and the combined route seam in `apps/web/src/fork/settings/useForkSettingsResetPlan.ts`
...
- Goal:
  - verify the full sync candidate before opening `sync/upstream-20260426 -> main`
- Planned automation:
