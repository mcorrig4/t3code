...
try {
  ...
  process.stdout.write("[sync-phase-2] fetching manifest from local dev web...\n");
  const manifestResponse = await fetch(`${localWebUrl}/manifest.webmanifest`, {
    signal: AbortSignal.timeout(5_000),
  });
  const manifestResult = {
    ok: manifestResponse.ok,
    text: await manifestResponse.text(),
  };

  if (!manifestResult.ok) {
    throw new Error("manifest.webmanifest did not load successfully");
  }
  ...
  process.stdout.write(`[sync-phase-2] PASS ${baseUrl}\n`);
} finally {
  await browser.close();
}
