...
describe("fork settings persistence", () => {
  ...
  it("also migrates fork-only keys from legacy client settings storage", () => {
    window.localStorage.setItem(
      LEGACY_CLIENT_SETTINGS_STORAGE_KEY,
      JSON.stringify({
        pushNotificationsEnabled: true,
    ...
    expect(migrateLegacyForkSettings()).toEqual({
      pushNotificationsEnabled: true,
      suppressCodexAppServerNotifications: false,
    });
  });
...
describe("buildForkSettingsResetPlan", () => {
  it("includes fork-only labels alongside upstream labels", () => {
    expect(
      buildForkSettingsResetPlan({
        upstreamDirtyLabels: ["Theme", "Providers"],
        forkSettings: {
          pushNotificationsEnabled: true,
          suppressCodexAppServerNotifications: false,
        },
      ...
      allDirtyLabels: ["Theme", "Providers", "Push notifications"],
      hasChanges: true,
    });
  });
  ...
  it("composes the upstream reset handler with fork reset behavior", () => {
    ...
    expect(resetPlan.allDirtyLabels).toEqual(["Theme", "Codex session overrides"]);
