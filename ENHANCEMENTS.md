...
- Status: active | Added: 2026-03-16 | Updated: 2026-03-25
- Upstream impact: medium
- Why: Provider callback state doesn't survive restarts, so stale approval/input prompts need cleanup. Fork keeps an isolated debug sidecar with floating panel, toast breadcrumbs, and global error capture.
- Seam: `apps/web/src/routes/__root.tsx` → `apps/web/src/fork/bootstrap/ForkRootSidecars.tsx`; `apps/web/src/components/ChatView.tsx` (breadcrumbs)
- Files:
  ...
  - `apps/web/src/routes/__root.tsx`, `apps/web/src/debug/userInputDebug.ts`, `apps/web/src/debug/UserInputDebugSidecar.tsx`
  - `apps/web/src/components/debug/UserInputDebugPanel.tsx`, `apps/web/src/components/ChatView.tsx`, `apps/web/src/components/chat/ComposerPendingUserInputPanel.tsx`
  - `apps/web/src/fork/bootstrap/ForkRootSidecars.tsx`, `apps/web/src/fork/bootstrap/rootDebug.ts`
- Upstream replacement trigger: upstream adds stale-prompt cleanup and developer diagnostics
- Verify: `bun run test src/session-logic.test.ts src/provider/Layers/ProviderService.test.ts`; load with `?debugUserInput=1` — debug panel appears; reproduce stale question after restart — old prompt cleared
- Rollback: revert stale-request cleanup in files above; remove sidecar mount for debug-only removal
- Notes:
  - 2026-03-26: Introduced `ForkRootSidecars.tsx` and `rootDebug.ts` so `__root.tsx` mounts a single fork debug sidecar seam; added lazy debug logging with `logUserInputDebugLazy`.
